...
fn main() -> ! {
    ...
    // Initialize the driver (verifies chip ID)
    let mut driver = Driver::new(i2c).unwrap();

    // Trigger a full recalibration
    driver.calibrate().unwrap();

    // Wait until a touch is detected
    loop {
    ...
    // Inspect _all_pads via debugger breakpoint here
    loop {}
}
